...
class SavedImagesRepoImpl(private val context: Context) : SavedImageRepo {
    ...
    override suspend fun loadSavedImages(): List<Pair<File, Bitmap>>? {
        ...
        dir.listFiles()?.let { data ->
            data.forEach { file ->
                savedImages.add(Pair(file, getPreviewedBitmap(file)))
            }
            return savedImages
        } ?: return null
    }

    private fun getPreviewedBitmap(file: File) : Bitmap {
